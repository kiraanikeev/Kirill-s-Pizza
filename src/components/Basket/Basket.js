...
function Basket() {
  ...
  function addBasket(pizza) {
    ...
    if (exist) {
      setBasket(
        basket.map((item) =>
          item.id === pizza.id ? { ...exist, amount: exist.amount + 1 } : item
        )
      );
    } else {
  ...
  const totalPrice = basket.reduce(
    (accumulator, currentValue) =>
      accumulator + currentValue.amount * currentValue.cost,
    0
  );
  ...
  return (
    <div className={styles.basket}>
      ...
      <NavLink to="/menu" className={styles.addPizzas}>
        add other pizzas
      </NavLink>
      <div>
        {basket.map((item) => {
            ...
            <div className={styles.goods} key={item.id}>
              <div className={styles.flex}>
                <img
                  ...
                  src={
                    process.env.PUBLIC_URL + `/Imgs/sort_of_pizza/${item.image}`
                  }
                />
